...
const ProfilePage = () => {
  ...
  const handleUpdateProfile=async(e)=>{
    ...
    try{
      ...
      console.log("Respnse", response, formData)
    }catch(error){
    ...
    <div className='bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleUpdateProfile}>
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" readOnly name={user?.email || ''} className='w-full h-10 pl-10 pr-3 py-2 border border-slate-200 rounded-lg bg-slate-50 text-slate-500 disabled: cursor-not-allowed ' disabled />
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-slate-200">
            ...
            <div className="space-y-4">
              ...
              icon={Building} type="text" value={formData.businessName} onChange={handleInputChange} placeholder="Your company LLC"
              />
              <TextareaField label="Address" name="address" icon={MapPin} value={formData.address} onChange={handleInputChange} placeholder="123 Main St, Anytown,India"/>
              <InputField label="Phone" name="phone"
              icon={Phone} type="tel" value={formData.phone} onChange={handleInputChange} placeholder="+91-12345 54321"
              />
            </div>
          </div>
        </div>
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-end">
          ...
          >
            {isUpdating ? <Loader2 className='size-5 mr-2 animate-spin'/>:null}
